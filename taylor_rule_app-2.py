...
def load_data(file):
    ...
    if name.endswith(".csv"):
        ...
        try:
            ...
            st.error(
                "This file is .xlsx and requires the **openpyxl** package.\n\n"
                "Fix options:\n"
                "1) Install it locally and re-run: `pip install openpyxl`\n"
                "2) Re-upload the data as a **CSV** file."
            )
            return None
        ...
        try:
            ...
            st.error(
                "Reading legacy **.xls** files may require the **xlrd** package (<=1.2.0) or converting to CSV.\n\n"
                "Fix options:\n"
                "1) Convert the file to **CSV** and re-upload.\n"
                "2) Or install a compatible reader for .xls (e.g., `pip install xlrd==1.2.0`) and run locally."
            )
            return None
    else:
...
if uploaded is not None:
    ...
    pi_label_to_col = {
        "Headline inflation": "pi_headline",
        "Core CPI": "pi_core_cpi",
        "Core PCE": "pi_core_pce",
    }
    pi_label = st.radio(
        "Inflation series to use in the Taylor rule",
        options=list(pi_label_to_col.keys()),
        horizontal=True,
    )
    pi_col = pi_label_to_col[pi_label]
    ...
    for t in range(LAG, len(ff_model)):
        ...
        if np.isnan(prior):
            ...
            ff_model[t] = rho * prior + (1 - rho) * baseline
    ...
    col2.metric("MSE", f"{mse:.3f}" if np.isfinite(mse) else "n/a")
    col3.metric("RMSE", f"{rmse:.3f}" if np.isfinite(rmse) else "n/a")
    col4.metric("MAE", f"{mae:.3f}" if np.isfinite(mae) else "n/a")
