...
# -------- Engines (variable h_t) --------
def results_variable_h(
    S_paths, S0, DF_d,
    r_d, r_f,
    costs_dc, revenue_fc,
    spread_bps_per_year,
    h_vec, strategy  # 'all_at_t0' or 'roll_one_year'
):
    n_sims, T_plus_1 = S_paths.shape
    T = T_plus_1 - 1
    h_vec = np.clip(np.asarray(h_vec, float), 0.0, 1.0)
    if h_vec.shape[0] != T:
        raise ValueError("h_vec length must equal T.")
    ...
    if strategy == "all_at_t0":
        ...
        for t in range(1, T+1):
            ...
            spot_t = np.maximum(S_paths[:, t], 1e-12)
            dc_rev_t[:, t-1] = hedged_fc * bid_prev_t + unhedged_fc * spot_t
    else:
    ...
    out = {
        ...
        "mean_dc_rev_by_year": np.nanmean(dc_rev_t, axis=0),
    }
    return out

# -------- UI (advanced) --------
st.set_page_config(page_title="Currency Hedging — Advanced (variable h_t)", layout="wide")
st.title("💱 Currency Risk Hedging — Advanced (variable $h_t$)")

with st.expander("Show math / notation"):
    st.latex(r"F_{t\to m} = S_t \cdot \frac{(1+r_d)^{m-t}}{(1+r_f)^{m-t}}")
    st.latex(r"\mu = \ln(1+\pi_{\Delta}) - \tfrac{1}{2}\sigma^2,\quad \mathbb{E}\!\left[\frac{S_t}{S_{t-1}}\right]=1+\pi_{\Delta}")
    st.latex(r"h_t \in[0,1],\ \ \text{hedged FC at } t = h_t \cdot \text{revenue}_t")
    st.markdown(
        r"""
**Bid–ask spread per tenor**: \( s_{\text{total}} = k \cdot s_{\text{per-year}} \) bps for a \(k\)-year forward.
        """
    )
...
per_year_df = st.data_editor(
    ...
    column_config={
        "Hedge h_t (%)": st.column_config.NumberColumn(
            "Hedge h_t (%)", help="Fraction of that year's revenue hedged via forwards.",
            min_value=0.0, max_value=100.0, step=1.0, format="%.1f"
        )
    },
)
h_vec = per_year_df["Hedge h_t (%)"].to_numpy(float)/100.0
...
if simulate_btn:
    ...
    zero_vec = np.zeros(T, dtype=float)
    unhedged = results_variable_h(
        S_paths, S0, DF_d, r_d, r_f, costs_dc, revenue_fc, spread_bps_per_year,
        h_vec=zero_vec, strategy="all_at_t0"
    )
    # Two strategies with user h_t
    all0 = results_variable_h(
        S_paths, S0, DF_d, r_d, r_f, costs_dc, revenue_fc, spread_bps_per_year,
        h_vec=h_vec, strategy="all_at_t0"
    )
    roll = results_variable_h(
        S_paths, S0, DF_d, r_d, r_f, costs_dc, revenue_fc, spread_bps_per_year,
        h_vec=h_vec, strategy="roll_one_year"
    )
    ...
    # Overlayed PV-profit hist (two hedge strategies)
